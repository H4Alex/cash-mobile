# S1-E3 — Mapa de Regras de Negócio: Consumer ADMIN
...
| Usuário já autenticado e `tipo_global === 'admin'` | **NÃO** há redirect automático; o login é resubmetido e navega para `/` após sucesso | `src/pages/LoginPage.tsx:27` |
...
| Senha curta | — | "Mínimo 6 caracteres" | Inline | Até corrigir |
...
| 11 | Rodapé "Atualizado agora" em cada card | Texto com ícone `ArrowUpRight` | Sempre (estático) | — | Visível |
...
- `staleTime`: 30 segundos
- Parâmetros: `{ page, limit: 20, entidade?: string }`
...
| Paginação | `page` | Numérico | 1..N | 1 |
| Limite por página | `limit` | Fixo | 20 | 20 |
...
| Botão de exportação | Não implementado | admin | Design da tela |
...
| `inadimplente` | Inadimplente | `warning` (amarelo) [INFERIDO — verificar com a equipe] | → ativa, → cancelada | [Não exibido na UI admin atualmente] |
