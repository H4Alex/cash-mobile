# S4-E3 — Mapa de Regras de Negócio: Consumer MOBILE (Atualizado)

> Atualizado a partir de S1-E3 com base no diff S4-E1 e Swagger changelog S4-E2.
> Etapa 3c do pipeline de documentação. Data: 2026-03-03.
...
| # | Elemento | Tipo | Exibir quando | Ocultar quando | Padrão |
|---|----------|------|---------------|----------------|--------|
| 1 | Menu "Campanhas" | TouchableOpacity | perfil ∈ {proprietario, gestor} | perfil ∈ {operador, vendedor} | — |
| 2 | Menu "Vendas" | TouchableOpacity | perfil ∈ {proprietario, gestor, operador} | perfil === vendedor | — |
| 3 | Menu "Contestações" | TouchableOpacity | perfil ∈ {proprietario, gestor} | perfil ∈ {operador, vendedor} | — |
| 4 | Menu "Relatórios" | TouchableOpacity | perfil ∈ {proprietario, gestor} | perfil ∈ {operador, vendedor} | — |
| 5 | Menu "Configurações" | TouchableOpacity | perfil === proprietario | perfil !== proprietario | — |
...
- **Status**: Função `unregisterPushDevice()` disponível mas **NÃO wired em logout**
- **Impacto**: Potencial push para dispositivos deslogados
- **Nota**: Aguardando integração futura no fluxo de logout
...
| CC-7 | Push unregister | src/services/mobile.notification.service.ts | unregisterPushDevice() (not wired) | Cross-cutting #7 |
...
| # | Item | Status | Origem |
|---|------|--------|--------|
| M1 | HistoricoScreen endpoint `/historico` vs `/extrato` | TODO preservado no código | S1-E3 |
| M3-M6 | 4 itens [AGUARDANDO VALIDAÇÃO] não tocados em S2/S3 | Não resolvidos | S1-E3 |
| CC-7 | Push unregister em logout | Função disponível mas não integrada | S4 Cross-cutting |
...
> **S4 ALTERAÇÃO**: Permissões de perfil resolvidas de [INFERIDO] para implementação real (role-gating). Ref Diff: S2-E5 Impl#2. `biometricEnrolled` agora gerenciado via API (enroll + unenroll). Ref Diff: S2-E5 C1.
...
> **S4 ALTERAÇÃO**: Status `encerrada` renomeado para `finalizada` em todo o sistema. [INFERIDO] resolvido — `finalizada` é estado final confirmado. Ref Diff: S2-E5 C4.
