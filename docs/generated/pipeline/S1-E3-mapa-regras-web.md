# S1-E3 — Mapa de Regras de Negócio: Consumer WEB
...
| # | Ação | Endpoint | Método | Payload | Validações | Feedback |
|---|------|----------|--------|---------|------------|----------|
| 1 | Enviar email (step 1) | `/auth/forgot-password` | POST | `{ email }` | Zod `emailStepSchema`: email required + formato válido | Sucesso: avança para step `codigo` |
| 2 | Validar código (step 2) | `/auth/reset-password` | POST | `{ email, token }` | Zod `codeStepSchema`: 6 dígitos numéricos | Sucesso: avança para step `nova-senha` |
| 3 | Redefinir senha (step 3) | `/auth/reset-password` | POST | `{ email, token, senha }` | Zod `newPasswordStepSchema`: min 8 + upper + lower + digit + match | Sucesso: avança para step `sucesso` |
...
| Regra # | Arquivo | Método | Evidência |
|---------|---------|--------|-----------|
| Step 1 | `src/hooks/useRecuperacaoWizard.ts` | `handleEnviarEmail()` | Chama API forgot-password (mock com TODO para integrar) |
| Step 2 | `src/hooks/useRecuperacaoWizard.ts` | `handleValidarCodigo()` | Chama API verify-code (mock com TODO para integrar) |
| Step 3 | `src/hooks/useRecuperacaoWizard.ts` | `handleRedefinirSenha()` | Chama API reset-password (mock com TODO para integrar) |
...
| 5 | StatusSummaryCard | Card lateral | Dados carregados | Loading | Skeleton |
...
| 7 | Ações em lote (aprovar/exportar/excluir) | — | — | — | — | [INFERIDO — verificar com a equipe] |
...
|--------|------|---------|---------------|
| Tipo | Select | all, credited, redeemed | Client |
| Status | Select | all, pending, credited, processing, expired | Client |
...
| Endpoint | Método | Campos Utilizados | Dependências |
|----------|--------|-------------------|--------------|
| `GET /dashboard/stats` | GET | `total_transacoes`, `total_cashback_concedido`, `cashbacks_validos`, `cashbacks_utilizados`, `cashbacks_expirados` | Auth + empresa.scope; reutiliza `useDashboardStats()` |
...
- **Error:** Tratado pelo hook `useDashboardStats()`
...
| Regra # | Arquivo | Método | Evidência |
|---------|---------|--------|-----------|
| Métricas | `src/pages/RelatoriosPage.tsx` | render | `useDashboardStats()` reutiliza dados do DashboardPage |
...
| Regra # | Arquivo | Método | Evidência |
|---------|---------|--------|-----------|
| Listagem | `src/pages/AuditoriaPage.tsx` | render | `useAuditoria(params)` com `limit: 50` |
...
| # | Ação | Endpoint | Método | Payload | Validações | Feedback |
|---|------|----------|--------|---------|------------|----------|
| 1 | Criar usuário | `POST /usuarios` | POST | `{ nome, email, senha: 'temp123456', perfil: 'gestor'\|'operador'\|'vendedor' }` | `userSchema` (Zod) | Sucesso: `'Usuário cadastrado com sucesso!'` / Erro: `'Erro ao cadastrar usuário.'` |
...
| Senha temporária | `src/pages/configuracoes/UsuariosTab.tsx` | `onSubmitUser()` | Novo usuário recebe `senha: 'temp123456'` [INFERIDO — verificar com a equipe: senha hardcoded] |
